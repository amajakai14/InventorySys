...
import com.excellence.dqube.base.XMLParserAPI;
	...
	public class BLJFourWeeksGoodsSales extends BLJsonMap4DB implements IBisinessLogic {
		...
		public void setModel(IModel model){
			...
			super.pageTitle = "BLJFourWeeksGoodsSales";

			super.defaultSQL = "Select inv.MaterialID, m.MaterialName, InventoryOut, Date\n"
					+ "From Inventory_Count inv\n"
					+ "LEFT JOIN Material_M m\n"
					+ "on inv.MaterialID = m.MaterialID\n"
					+ "Where (Date = ?) or (Date = ?) or (Date = ?) or (Date = ?);";
	...
	public boolean innerLogic(){
		...
		String Week1 = firstParam("Week1");
		String Week2 = firstParam("Week2");
		String Week3 = firstParam("Week3");
		String Week4 = firstParam("Week4");
		//格納モデルをインスタンス化
		outModel = new PModel();
		...
		try{
			...
			$pstm.setDate(1, java.sql.Date.valueOf(Week1));
			$pstm.setDate(2, java.sql.Date.valueOf(Week2));
			$pstm.setDate(3, java.sql.Date.valueOf(Week3));
			$pstm.setDate(4, java.sql.Date.valueOf(Week4));
