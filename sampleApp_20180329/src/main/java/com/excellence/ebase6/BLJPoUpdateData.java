 ...
 *
 */
public class BLJPoUpdateData extends BLJsonMap4DB implements IBisinessLogic {
	...
	public void setModel(IModel model){
		...
		super.pageTitle = "BLJPoUpdateData";

		//super.defaultSQL = "INSERT INTO ITEM_MST VALUES (?, ? ,? ,? ,? ,?)";
		super.defaultSQL = "UPDATE PurchaseOrder_T SET OrderQuan = ?, DeliverDate = ?, PoRemark = ? WHERE PoNumber = ?";
	...
	public boolean innerLogic(){
		...
		String order = firstParam("order");
		String delidate = firstParam("delidate");
		...
		//格納モデルをインスタンス化
				...
				try{
					...
					$pstm.setInt(1, Integer.parseInt(order));
					$pstm.setDate(2, java.sql.Date.valueOf(delidate));
					$pstm.setString(3, poremark);
