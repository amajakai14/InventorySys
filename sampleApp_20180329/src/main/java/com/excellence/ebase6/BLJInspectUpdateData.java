 ...
 *
 */
public class BLJInspectUpdateData extends BLJsonMap4DB implements IBisinessLogic {
	...
	public void setModel(IModel model){
		...
		super.pageTitle = "BLJInspectUpdateData";

		//super.defaultSQL = "INSERT INTO ITEM_MST VALUES (?, ? ,? ,? ,? ,?)";
		super.defaultSQL = "UPDATE Inspect_Test SET InspectCount = ?, BadCount = ?, InspectShort = ?, InspectRemark = ?  WHERE InspectNo = ?";
	...
	public boolean innerLogic(){
		...
		//格納モデルをインスタンス化
				...
				try{
					...
					$pstm.setInt(1, Integer.parseInt(realcount));
					$pstm.setInt(2, Integer.parseInt(badcount));
					$pstm.setInt(3, Integer.parseInt(shortcount));
					$pstm.setString(4, checkremark);
					$pstm.setInt(5, Integer.parseInt(id));

					$pstm.execute();

					if($recode == null){
						sql="Select InspectNo, m.MaterialName, m.Unit, m.Supplier, p.OrderQuan, InspectCount, BadCount, InspectShort, ExpDate,InspectDate, InspectRemark\n"
								+ "From Inspect_Test i\n"
