 ...
 *
 */
public class BLJSupplyData extends BLJsonMap4DB implements IBisinessLogic {
	...
	public void setModel(IModel model){
		...
		super.pageTitle = "BLJSupplyData";

		super.defaultSQL = "INSERT INTO PurchaseOrder_T (MaterialID, PoDate, OrderQuan, DeliverDate, PoRemark) VALUES (?, CURDATE(), ?, ?, ?)";
	...
	public boolean innerLogic(){
		...
		String ord = firstParam("ord");
		String scheduled = firstParam("scheduled");
		String poremark = firstParam("poremark");
		//格納モデルをインスタンス化
		outModel = new PModel();
		...
		try{
			...
			$pstm.setInt(2, Integer.parseInt(ord));
			$pstm.setDate(3, java.sql.Date.valueOf(scheduled));
			$pstm.setString(4, poremark);

			$pstm.execute();

			if($recode == null){
				sql="select * from PurchaseOrder_T;";
				setSQL(sql);
